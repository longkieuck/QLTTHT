--TRIGGER
--update muc hoc phi cho hoc vien sau khi dang ki them lop hoc moi(muc hoc phi toi da la 3)
CREATE TRIGGER UPDATE_MUCHOCPHI
...
BEGIN
	...
	DECLARE @MAMHP INT;
	SET @MAMHP=(SELECT MAMHP
				FROM HOCVIEN
				WHERE MaHV=@MAHV)
	IF(@MAMHP < 3)
	BEGIN
		UPDATE HOCVIEN
		SET MaMHP = HOCVIEN.MaMHP + 1
		WHERE MAHV = @MAHV
	END
END
...
--Trigger khi học viên xoá lớp đang học thì cập nhật lại mức học phí
CREATE TRIGGER UPDATE_MUCHOCPHI2
...
BEGIN
	...
	SET @MUCUUDAI=(SELECT MUCUUDAI FROM HOCVIEN,MUCHOCPHI
				   WHERE HOCVIEN.MaHV=@MAHV AND MUCHOCPHI.MaMHP=HOCVIEN.MaMHP
				  )
	...
	DECLARE @HP1BUOI INT;
	SET @HP1BUOI=(SELECT HP1BUOI FROM LOPHOC,BUOIHOC
